...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		 ...
		 */
         ...
         /* Determine file type and call filldir() */
         ...
         if (entry_oi->oi_ftype == OSPFS_FTYPE_REG)
            ...
            ftype = DT_LNK;
         ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos, dir_inode->i_ino, ftype);
...
direct_index(uint32_t b)
{
    ...
    /* Index into indirect block, which is in an array of indirect blocks */
...
add_block(ospfs_inode_t *oi)
{
    ...
    if (index == -1)    /* Direct block */
    {
        ...
        memset(ospfs_block(dir_block), 0, OSPFS_BLKSIZE);
    }
    else if (index == 0)        /* Indirect block */
    {
        uint32_t *arr;
        /* Allocate indirect block if not already */
        if (oi->oi_indirect == 0)
        {
        ...
        /* Allocate direct block, then set indirect_block_array[n] to direct */
        dir_block = allocate_block();
        if (dir_block == 0)
        {
        ...
        arr = ospfs_block(oi->oi_indirect);
        arr[direct_index(n)] = dir_block;
        memset(ospfs_block(dir_block), 0, OSPFS_BLKSIZE);
    }
    else if (n < OSPFS_MAXFILEBLKS)         /* Double indirect block */
    {
        uint32_t *arr, *arr_indirect;
        ...
        /* Allocate a single indirect block if needed */
        arr_indirect = ospfs_block(oi->oi_indirect2);
        if (arr_indirect[index] == 0)
        {
            allocated[0] = allocate_block();
            if (allocated[0] == 0)
            {
            ...
            arr_indirect[index] = allocated[0];
            memset(ospfs_block(allocated[0]), 0, OSPFS_BLKSIZE);
        }
        ...
        arr = ospfs_block(arr_indirect[index]);
        arr[direct_index(n)] = dir_block;
        memset(ospfs_block(dir_block), 0, OSPFS_BLKSIZE);
    }
    else
...
remove_block(ospfs_inode_t *oi)
{
	// current number of blocks in file
	uint32_t n = ospfs_size2nblocks(oi->oi_size);
    int32_t index = indir_index(n - 1);
    int32_t dir_index = direct_index(n - 1);
    ...
    {
        uint32_t *arr;
        /* Get pointer to last block in direct-block-array and free it */
        if (oi->oi_indirect == 0)
            return -EIO;

        arr = ospfs_block(oi->oi_indirect);
        free_block(arr[dir_index]);
        arr[dir_index] = 0;
        
        /* If all blocks in direct-block-array are 0, free indirect block */
        if (dir_index == 0)
        {
    ...
    {
        uint32_t *arr, *arr_indirect;
        /* Get pointer to indirect block in indirect-block-array,
           and free block in that blocks' direct-block-array. */
        if (oi->oi_indirect2 == 0)
            return -EIO;
        arr_indirect = ospfs_block(oi->oi_indirect2);
        arr = ospfs_block(arr_indirect[index]);
        ...
        /* Free indirect block, and double indirect block, if empty */
        if (dir_index == 0)
        {
            free_block(arr_indirect[index]);
            arr_indirect[index] = 0;
            if (index == 0)
            {
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	...
	while (ospfs_size2nblocks(oi->oi_size) > ospfs_size2nblocks(new_size)) {
        ...
        if (retval != 0)        /* Only possible when if old_size == 0 */
            return retval;
	}
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
        ...
        n = OSPFS_BLKSIZE - offset;         /* bytes left in block */
        if (n > count - amount)
            n = count - amount;
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
    ...
    {
        int retval = change_size(oi, *f_pos + count);
        if (retval != 0)
            goto done;
    }
	...
	while (amount < count && retval >= 0) {
        ...
        /* Identical to read(), except use copy_from_user() */
        offset = *f_pos % OSPFS_BLKSIZE;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
    ...
    /* Add one block and return the first dentry */
    ret = add_block(dir_oi);
    if (ret < 0)
        return ERR_PTR(ret);

#ifndef NDEBUG
    eprintk("Added block and end of create_blank_direntry()\n");
#endif
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
    ...
    /* Create new link in the directory, then copy dst_dentry info to the link */
    ...
    // dst_dentry->d_inode->i_ino = src_dentry->d_inode->i_ino;     // d_inode = 0x00, so don't bother setting it
    ...
    src_oi->oi_nlink++; /* Remember to increment link count */
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
    ...
    /* Find an empty inode (blocks comprise an array of inodes) */
    for (entry_ino = 0, inode = ospfs_block(ospfs_super->os_firstinob);
    ...
    /* Create the direntry, and then initialize the inode */
    link->od_ino = entry_ino;
    memcpy(link->od_name, dentry->d_name.name, dentry->d_name.len);
    link->od_name[dentry->d_name.len] = '\0';
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
    ...
    /* Not exactly identical to create() */
    inode->oi_size = strlen(symname);
    inode->oi_ftype = OSPFS_FTYPE_SYMLINK;
    inode->oi_nlink = 1;
    memcpy(inode->oi_symlink, symname, strlen(symname));
    inode->oi_symlink[strlen(symname)] = '\0';
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
    ...
    // NOT NECESSARY - conditional symlinks are not tested
